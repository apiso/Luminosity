...
def mass_loss(filename, prms, td = 3e6, tol = 1e-24, n = 500, nMpoints = 5000):
    ...
    while(np.abs(Ecool - Eevap)) >= 0 and flag == 0:
        ...
        #lum = x[1]   
            ...
            ynew = odeint(f, [x[0], Tc, model.Mco, x[1], 0, 0], [rnew[0], rnew[-1]])  
        ...
        Pctry, Ltry = param2.Pc[i], param2.L[i]
        x0 = (Pctry, Ltry)  
        match = root(delta, x0)
        if match.success == False:
            ...
            Pcmatch, Lmatch =  match.x
    
        ynew = odeint(f, [Pcmatch, Tc, model.Mco, Lmatch, 0, 0], rnew)
    ...
    return prof2, param2, time, Ecool, Eevap, i
