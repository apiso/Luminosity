...
def Ltop(Mi, L1, L2, n, tol, prms = prms):
    ...
    def f(x, r):
        ...
        return np.array([ - G * x[1] * x[0] / (r**2 * prms.R * x[1]), \
    ...
    def delta(lum):
        ...
        y = odeint(f, [prms.Pd, prms.Td, lum], [RHill(Mi, prms.a), prms.rco])
        ...
        deltaL = 4 / np.pi * np.arctan(Mc1 / prms.Mco) - 1
...
def shoot(Mi, L1, L2, n, tol, prms = prms, delad2 = prms.delad, Tswitch = 500):
    ...
    Lpluserror = Ltop(Mi, L1, L2, n, tol, prms, delad2, Tswitch)
    ...
    def f(x, r):
        ...
        if x[1] <= Tswitch:
            ...
            prms = prms._replace(delad = delad2, gamma = gammafn(delad2), \
                Cv = Cvfn(prms.Y, delad2))

        else:

            prms = prms._replace(delad = delad, gamma = gammafn(delad), \
    ...
    return r, P, T, m, rho, delrad, Eg, U, Mi / Me, Mcb / Me, MB / Me, rcb / Re, \
